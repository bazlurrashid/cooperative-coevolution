/*	A Cooperative Co-Evolution for Feature Selection with Random Feature Grouping in every generation*/
package ccfsrfg2;
...
public class Driver{	// start of the Driver class
	...
	public static final int NO_OF_GENERATION = 10000, NO_OF_GENES = 10, SUBPOP_SIZE = 30;	// // Specify here the maximum number of generation, number of features (i.e., NO_OF_GENES) in each subpopulation, and size of each 
	...
	public static int SOLUTION_SIZE, NO_OF_SUBPOPULATION;	// The total number of attributes in the dataset and Number of genes in each individual
	public static String TRAIN_DATASET, TEST_DATASET, TRAIN_DATASET_NAME, TEST_DATASET_NAME;
	public static int NUMBER_OF_TRAIN_SAMPLES, NUMBER_OF_TEST_SAMPLES;	// The number of samples in train and test sets

	public static String MODEL_SAVE = "C:\\Users\\Bazlur\\Desktop\\feature-selection\\output\\CCFSRFG-2\\trained.model";
	public static String DIRECTORY = "C:\\Users\\Bazlur\\Desktop\\feature-selection\\output\\CCFSRFG-2\\";	// The directory where the outputs will be written
	public static String CSV_GEN = "", CSV_SUB = "", CSV_IND = "", RTF_GEN = "", RTF_SUB = "", RTF_IND = "", RTF_DS = "";
	
	public static DataSource trainSource, testSource;	// The source of the dataset, which will use the TRAIN_DATASET and TEST_DATASET
	public static Instances trainData, testData;	// Instances or samples from the TRAIN_DATASET and TEST_DATASET
    ...
    public static void main(String[] args) throws Exception {	// Start of main() function
        ...
        jTextArea.append("\nCCFSRFG-2 starts at: " + formatter.format(startDate) + "\n");
		...
		try {
			...
			while(generationNumber < NO_OF_GENERATION) {
				...
				contextVector.findRandomCollaboratorsGenerations(subPopulation);
		...
		jTextArea.append("\nCCFSRFG-2 stops at: " + formatter.format(endDate) + "\n");
		jTextArea.append("\nCCFSRFG-2 execution completed successfully!!!");
	...
	public static void evolveSubPopulations() throws Exception {
		...
		for(int x = 0; x < NO_OF_SUBPOPULATION; x++) {	
			individual = subPopulation[x].getIndividuals()[0];
			subPopulation[x].setIndividuals(0, individual.getGenes(), individual.getAttributeIndices());
			attributeIndices = new int[sizeofEachSubPop[x]];
			for(int y = 0; y < sizeofEachSubPop[x]; y++) {
				attributeIndices[y] = Driver.groupedIndices.get(x).get(y);
			}
			for(int z = 1; z < Driver.SUBPOP_SIZE; z++) {
				individual = subPopulation[x].getIndividuals()[z];
				subPopulation[x].setIndividuals(z, individual.getGenes(), attributeIndices);
			}
		}
		for(int x = 0; x < NO_OF_SUBPOPULATION; x++) {
	...
	public static void defineFileNamesWithDateTime() {
        ...
        CSV_GEN = DIRECTORY + "CCFSRFG_2_" + TRAIN_DATASET_NAME + "_" + classificationModel + "_gen_" + strDate + ".csv";
        RTF_DS = DIRECTORY + "CCFSRFG_2_" + TRAIN_DATASET_NAME + "_"  + classificationModel + "_DS_" + strDate + ".rtf";
	}
	...
	public static void writeDSBestIndividual(Individual bestIndvidual) throws IOException {
		...
		dsRTFWriter.append("CCFSRFG-2 starts execution at: " + formatter.format(startDate) + "\n");
		...
		dsRTFWriter.append("\nCSV and RTF files created successfully!!! at \n\n" + CSV_GEN + "\n" + RTF_DS + "\n");
		dsRTFWriter.append("\nCCFSRFG-2 ends execution at: " + formatter.format(endDate) + "\n");
