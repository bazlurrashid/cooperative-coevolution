...
public class ClassPredictor {
	public static String DIRECTORY = "D:\\feature-selection\\output\\rare-anomaly-detection\\";
    ...
    public static SimpleDateFormat formatter; public static Date startDate, endDate;	public static Stopwatch timer;
    
	public static String[] classValue, actual, predString;	public static double TP, TN, FP, FN, TPR, TNR, FPR, FNR;
	public static double[] CTPR, CTNR; 
	...
	public static void main(String[] args) throws Exception{
		...
		try {createAndInitiateFileWriters();
			...
			int[] classCorrect = null, classIncorrect = null, classTotal = null;
			...
			for (int j = 0; j < numTestInstances; j++){
				...
				genWriter.append(String.valueOf((unlabeled.instance(j).stringValue(unlabeled.numAttributes()-1))));						
				genWriter.append(COMMA_DELIMITER);
			...
			classTotal = new int[numTestInstances];
			...
			for (int i = 0; i < numClasses; i++){
				...
				classTotal[i] = classCorrect[i] + classIncorrect[i];
			}
			...
			for (int i = 0; i < numClasses; i++){
				...
				System.out.println("\nClass: "+ classValue[i] + ", Class Total:\t" + classTotal[i] + ", Correctly classified:\t" + classCorrect[i] + " (" + pctClassCorrect[i] + " or " + df.format(pctClassCorrect[i]) + ")" + "\tIncorrectly classified:\t\t\t" + classIncorrect[i] + " (" + pctClassIncorrect[i] + " or " + df.format(pctClassIncorrect[i]) + ")");
				dsRTFWriter.append("\nClass: "+ classValue[i] + ", Class Total:\t" + classTotal[i] + ", Correctly classified:\t" + classCorrect[i] + " (" + pctClassCorrect[i] + " or " + df.format(pctClassCorrect[i]) + ")" + "\tIncorrectly classified:\t\t\t" + classIncorrect[i] + " (" + pctClassIncorrect[i] + " or " + df.format(pctClassIncorrect[i]) + ")");
				dsRTFWriter.append(NEW_LINE_SEPARATOR);
			}
			...
			dsRTFWriter.append(NEW_LINE_SEPARATOR);
			System.out.println();
			int totalAnomalyCorrect = 0;
			CTPR = new double[numClasses]; 
			CTNR = new double[numClasses];
			for(int x = 0; x <numClasses; x++) {
				CTPR[x] = Double.valueOf(classCorrect[x]) / Double.valueOf(classTotal[x]);
				CTNR[x] = Double.valueOf(classIncorrect[x]) / Double.valueOf(classTotal[x]);
				System.out.println("TPR["+ classValue[x] +"]:" + "\t\t\t" + df.format(CTPR[x]) + "\t\t\tTNR["+ classValue[x] +"]:" + "\t\t\t" + df.format(CTNR[x]));
				dsRTFWriter.append(NEW_LINE_SEPARATOR);
				dsRTFWriter.append("TPR["+ classValue[x] +"]:" + "\t\t\t" + df.format(CTPR[x]) + "\t\t\tTNR["+ classValue[x] +"]:" + "\t\t\t" + df.format(CTNR[x]));	dsRTFWriter.append(NEW_LINE_SEPARATOR);
			}
			
			for(int x = 1; x < numClasses; x++) {
				totalAnomalyCorrect+= classCorrect[x];
			}
			
			double TPR_Anomalies;	
			TPR_Anomalies = Double.valueOf(totalAnomalyCorrect)/Double.valueOf(anomalyInstances);
			
			System.out.println("\nCorrectly classified anomalies = " + totalAnomalyCorrect + "\tTotal number of test anomaly instances = " + anomalyInstances);
			...
			System.out.println("CCFSRFG-1 Anomaly Detection stops at: " + formatter.format(endDate));
			System.out.println("CCFSRFG-1 Anomaly Detection execution completed successfully!!!");
			System.out.println("\n====== Total Time Elapsed for the Execution: " + String.valueOf(timer) + "\n");
			dsRTFWriter.append("CCFSRFG-1 Anomaly Detection stops at: " + formatter.format(endDate));	dsRTFWriter.append(NEW_LINE_SEPARATOR);
			dsRTFWriter.append("CCFSRFG-1 Anomaly Detection execution completed successfully!!!");	dsRTFWriter.append(NEW_LINE_SEPARATOR);
			dsRTFWriter.append("\n====== Total Time Elapsed for the Execution: " + String.valueOf(timer) + "\n");	dsRTFWriter.append(NEW_LINE_SEPARATOR);
		}	// end of try 	
		catch (Exception e) {
